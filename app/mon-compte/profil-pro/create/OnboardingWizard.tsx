            ...
            <div className="flex flex-col items-center gap-1">
              ...
              <span
                className={`text-[10px] font-semibold uppercase tracking-wide ${
                  active ? 'text-orange-primary' : done ? 'text-emerald-500' : 'text-gray-400'
                }`}
              >
                {label}
              </span>
...
function Step1({
  ...
  const handleNext = () => {
    setError('')
    if (form.name.trim().length < 2) { setError('Le nom doit faire au moins 2 caractères.'); return }
    if (!form.category) { setError('Veuillez choisir une catégorie.'); return }
    if (!form.city.trim()) { setError('Veuillez indiquer une ville.'); return }
    onNext()
  }
          ...
          >
            <span
              className={`relative inline-flex h-5 w-9 shrink-0 items-center rounded-full transition-colors ${
                form.phoneHidden ? 'bg-navy' : 'bg-gray-200'
              }`}
              role="switch"
            ...
            >
              <span
                className={`inline-block h-3.5 w-3.5 transform rounded-full bg-white shadow transition-transform ${
                  form.phoneHidden ? 'translate-x-4' : 'translate-x-1'
                }`}
              />
            </span>
    ...
    <div className="space-y-5">
      <section className="bg-white rounded-2xl border border-gray-100 p-6">
              ...
              >
                {selected && (
                  <div
                    className={`absolute top-3 right-3 w-5 h-5 rounded-full flex items-center justify-center ${
                      plan.highlight ? 'bg-indigo-primary' : 'bg-orange-primary'
                    }`}
                  >
                ...
                <span
                  className={`text-xs font-black mb-1 ${
                    plan.highlight ? 'text-indigo-primary' : 'text-orange-primary'
                  }`}
                >
                  {plan.tier}
                </span>
                    ...
                    <li key={f} className="flex items-start gap-2 text-xs text-gray-600">
                      <Check
                        ...
                        className={`shrink-0 mt-0.5 ${
                          plan.highlight ? 'text-indigo-primary' : 'text-emerald-500'
                        }`}
                      />
                      {f}
                    </li>
        ...
        <p className="text-xs text-gray-400 mt-4">
          🔒 Paiement sécurisé par carte via Stripe — annulation possible à tout moment.
        </p>
      </section>
    ...
    <div className="min-h-screen bg-[#f7f8fa] py-10 px-4">
      <div className="max-w-2xl mx-auto">
        <div className="text-center mb-8">
          <p className="text-orange-primary text-xs font-black uppercase tracking-widest mb-2">
            Nouveau professionnel
          </p>
          <h1 className="text-2xl sm:text-3xl font-black text-navy mb-2">Créez votre fiche pro</h1>
          <p className="text-sm text-gray-500 max-w-md mx-auto">
            Renseignez vos informations, choisissez votre abonnement et soyez visible en quelques
            minutes.
          </p>
        </div>
        <StepDots step={step} />
