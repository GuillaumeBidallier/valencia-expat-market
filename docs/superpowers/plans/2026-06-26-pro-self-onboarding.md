# Pro Self-Onboarding Wizard — Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Allow any logged-in user to create their own professional profile through a 4-step animated wizard that ends with a Stripe subscription checkout.

**Architecture:** Pure client-side state wizard — no data hits the DB until the user clicks "Payer et publier" on step 4. At that point, the pro record is created (`POST /api/pro/profile`), images are uploaded sequentially via the existing `/api/pro/upload` route, then the user is redirected to Stripe Checkout via the existing `/api/stripe/pro-subscription` route.

**Tech Stack:** Next.js App Router, Prisma, Stripe, Vercel Blob, Tailwind CSS, Zod, CSS keyframe animations (no framer-motion)

## Global Constraints

- No free tier — pros must pay to have a visible profile
- Slug auto-generated server-side from name (slugify + numeric suffix on collision)
- Animations via CSS keyframes only (framer-motion not installed)
- Images stored as `File` objects in React state during wizard, uploaded after pro record creation
- Use existing `proCategories` from `lib/proCategories.ts` for category select
- Use existing `ProPlan` type and `PRO_PLANS` from `lib/stripe.ts` for plan selection
- Tailwind classes follow existing project conventions (`bg-orange-soft`, `text-navy`, `text-orange-primary`, `rounded-2xl`, etc.)
- `NEXT_PUBLIC_APP_URL` env var used for Stripe redirect URLs

---

### Task 1: Add `POST /api/pro/profile` — create professional record

**Files:**
- Modify: `app/api/pro/profile/route.ts`

**Interfaces:**
- Consumes: `session.user.id`, request body `{ name, category, city, description?, phone?, whatsapp?, website?, phoneHidden?, zones?, plan: ProPlan }`
- Produces: `{ checkoutUrl: string }` on success — consumed by Task 3 wizard submit handler

- [ ] **Step 1: Add slugify utility inline and the POST handler**

Add this to `app/api/pro/profile/route.ts` after the existing imports and before the GET handler:
...
- [ ] **Step 2: Verify the existing GET and PATCH still present**

The file should now export `GET`, `PATCH`, and `POST`. Read it back and confirm all three handlers are present.

- [ ] **Step 3: Manual API smoke test**

Start dev server (`npm run dev`), log in as a user with no pro profile, then from browser console:
```js
const r = await fetch('/api/pro/profile', {
  method: 'POST',
  headers: { 'Content-Type': 'application/json' },
  body: JSON.stringify({ name: 'Test Pro', category: 'immobilier', city: 'Valencia', plan: 'premium_monthly' })
})
...
```bash
git add app/api/pro/profile/route.ts
git commit -m "feat: add POST /api/pro/profile — self-service pro creation + Stripe checkout"
```
...
**Files:**
- Create: `app/mon-compte/profil-pro/create/page.tsx`

**Interfaces:**
- Consumes: `auth()` session, `prisma.professional.findUnique`
- Produces: renders `<OnboardingWizard />` (created in Task 3) — import path `./OnboardingWizard`
...
```bash
git add app/mon-compte/profil-pro/create/page.tsx
git commit -m "feat: add /mon-compte/profil-pro/create page with auth + existing-pro guard"
```

---

### Task 3: Create `OnboardingWizard.tsx` — the full 4-step wizard

**Files:**
- Create: `app/mon-compte/profil-pro/create/OnboardingWizard.tsx`

**Interfaces:**
- Consumes: `POST /api/pro/profile` → `{ checkoutUrl }` (Task 1), `POST /api/pro/upload` (existing), `POST /api/stripe/pro-subscription` is NOT used here (Stripe starts inside POST /api/pro/profile)
- Consumes: `proCategories` from `@/lib/proCategories`
- Consumes: `ProPlan` type from `@/lib/stripe`
- Produces: standalone client component, no props

- [ ] **Step 1: Create the wizard file with all four steps**
...
- [ ] **Step 2: Verify Step 4 plan selection logic**

In Step4, the button checks `data.plan === plan.id`. The `PLANS` array only contains `premium_monthly` and `premium_plus_monthly`. The annual variants (`premium_annual`, `premium_plus_annual`) are not shown as primary buttons. If you want to add annual options, duplicate entries in PLANS with ids `premium_annual` / `premium_plus_annual`. For now, monthly is the default shown — acceptable per spec.

- [ ] **Step 3: Manual test in browser**

Navigate to `/mon-compte/profil-pro/create` as a logged-in user with no pro profile:
1. Fill step 1 — verify slug preview updates live as you type
2. Proceed through steps — verify slide animation plays
3. Add a logo on step 3 — verify preview appears
4. Select a plan on step 4 — verify card highlights
5. Click "Payer et publier" — verify loading state, then Stripe redirect

- [ ] **Step 4: Commit**

```bash
git add app/mon-compte/profil-pro/create/OnboardingWizard.tsx
git commit -m "feat: add 4-step pro onboarding wizard with animations and Stripe redirect"
```

---

### Task 4: Update empty state + create page guard

**Files:**
- Modify: `app/mon-compte/profil-pro/page.tsx`

**Interfaces:**
- No new interfaces — adds a `Link` to `/mon-compte/profil-pro/create`

- [ ] **Step 1: Replace empty state with CTA button**

In `app/mon-compte/profil-pro/page.tsx`, replace the `if (!pro)` block:
...
- [ ] **Step 2: Remove unused translation keys (optional cleanup)**

The `t('not_linked_title')`, `t('not_linked_body')`, `t('contact_us')` keys are no longer used. You may remove them from the translation files — or leave them, they cause no errors.

- [ ] **Step 3: Manual verification**

Visit `/mon-compte/profil-pro` as a logged-in user with no pro profile. Confirm:
- The new CTA "Créer ma fiche pro →" button appears
- Clicking it navigates to `/mon-compte/profil-pro/create`
- The wizard loads

- [ ] **Step 4: Commit**

```bash
git add app/mon-compte/profil-pro/page.tsx
git commit -m "feat: replace pro empty state with self-service onboarding CTA"
```
...
**Spec coverage:**
- ✅ Wizard 4 étapes (Identité, Présentation, Visuels, Plan)
- ✅ Slug auto-généré + preview live
- ✅ Animations CSS keyframes slide horizontal
- ✅ Upload images après création de la fiche
- ✅ Barre de progression animée
- ✅ Responsive mobile (single column, full-width)
- ✅ Bouton retour / Suivant / Payer et publier
- ✅ Redirect vers `/connexion` si non connecté
- ✅ Redirect vers dashboard si fiche déjà existante
- ✅ Zones d'intervention (tags)
- ✅ Toggle phoneHidden
- ✅ Cartes plan avec ring orange/indigo

**Placeholder scan:** Aucun TBD/TODO dans le plan.

**Type consistency:**
- `ProPlan` importé de `@/lib/stripe` — utilisé dans Task 1 (schema zod) et Task 3 (FormData, PLANS array)
- `checkoutUrl` retourné par Task 1 POST handler → consommé par Task 3 submit()
- `proCategories` importé de `@/lib/proCategories` — utilisé dans Step1
- Upload images après création : le `professional` record existe quand `POST /api/pro/upload` est appelé — route existante fonctionne sans modification
